...
async function upgrade_16(){
    let dbVersion = await getDBVersion();

    if(dbVersion < 16){
        await Query(`CREATE TABLE executed_exercises(
                        id SERIAL PRIMARY KEY,
                        invitation_id SERIAL REFERENCES invitations (id) ON DELETE CASCADE,
                        pass BOOLEAN,
                        display_time TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW(),
                        executed_time TIMESTAMP WITHOUT TIME ZONE,
                        exercise_id SERIAL REFERENCES exercises (id) ON DELETE CASCADE);
                    ALTER TABLE answers
                        DROP COLUMN invitation_id,
                        DROP COLUMN pass,
                        DROP COLUMN executed_time,
                        DROP COLUMN exercise_id,
                        ADD COLUMN executed_exercise_id SERIAL REFERENCES executed_exercises (id) ON DELETE CASCADE`);
        // set version number
        await incrementDBVersion();
    }
}

async function upgrade_17(){
    let dbVersion = await getDBVersion();
